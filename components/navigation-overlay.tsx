...
export default function NavigationOverlay({ items = DEFAULT_ITEMS, perItemDuration = 800, minShow = 600 }: { items?: string[], perItemDuration?: number, minShow?: number }) {
  ...
  return (
    <AnimationLoader open={open} items={items} perItemDuration={perItemDuration} maxDuration={perItemDuration} useAll={false} sizeClass="w-48 h-48" onFinish={() => setOpen(false)} />
  )
}
