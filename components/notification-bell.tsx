...
import { Bell, X, Check, ChevronRight, Clock, AlertTriangle, Sparkles, Trophy, Info } from 'lucide-react'
      ...
      <PopoverContent className="w-[min(420px,calc(100vw-32px))] p-3">
        ...
        <div className="max-h-[48vh] overflow-y-auto divide-y divide-gray-100">
          ...
          {notifications.map((n) => {
            ...
            }[type] || { bg: 'bg-slate-50', fg: 'text-slate-600', Icon: () => <Info className="w-4 h-4" /> }
            ...
            return (
              <div key={n.id} className={`flex items-start gap-4 p-4 rounded-lg border border-gray-100 shadow-sm transition-shadow ${n.ui_read ? 'bg-gray-50 opacity-80' : 'bg-white hover:shadow-md'}`}>
                <div className={`flex items-center justify-center ${icon.bg} rounded-lg w-12 h-12 shrink-0 border ${n.ui_read ? 'border-transparent' : 'border-gray-100'}`}>
                  <span className={`${icon.fg}`}>
                    <icon.Icon />
                  </span>
                </div>
