...
export async function POST(req: Request) {
  try {
    ...
    const totalPoints = basePoints + streakBonus + earlyBonus
    
    // Create upload record
    const uploadRecord = {
      challenge_id: challengeId,
      video_id: videoId,
      video_title: videoDetails.title || videoTitle,
    ...
    const challengeUpdates = {
      ...
      longest_streak: Math.max(challenge.longest_streak || 0, currentStreak),
    ...
    const achievements = await checkForAchievements(supabase, userRow.id, challengeId, currentStreak, completionPercentage)
    ...
    return NextResponse.json({
      ...
      message: `Upload tracked! ${isOnTime ? '✅ On time' : '⏰ Late'} - ${totalPoints} points earned`
    })
